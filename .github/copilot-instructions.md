...
6. **Cloud layer:** Terraform under `infra/terraform`, bootstrap module `infra/terraform/bootstrap-state`, `backend.tf.example` for S3 remote state, GitHub Actions **`test.yml`** on PRs, and **`deploy-aws.yml`** (push **`develop`/`staging`/`main`**: mandatory **S3 backend** + **OIDC** + **`terraform apply`** + Docker **ECR** push + second apply for image tags; GitHub Environments **`dev`/`test`/`prod`**).
