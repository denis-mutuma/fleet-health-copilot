...
- Container registry and deployed compute.
- Managed database.
...
- IAM role trusted by this repository.
- Least-privilege policy for the resources managed by Terraform.
...
2. **Artifact and container storage**: S3 artifacts bucket and ECR repositories.
3. **Compute**: simplest viable container runtime for web and orchestrator.
4. **Secrets**: Clerk keys and app configuration through a managed secret store.
5. **Data**: managed relational or file-backed store replacing local SQLite.
6. **Retrieval**: AWS S3 Vectors backend behind `RetrievalBackend`.
7. **Observability**: logs, metrics, and alarms for demo reliability.
...
The next safe implementation step is to add Terraform modules for ECR repositories and GitHub OIDC role scaffolding. That creates a deployment foundation without committing to the full compute design yet.
